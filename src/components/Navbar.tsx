  ...
  return (
    ...
    >
      <div className="mx-auto max-w-6xl px-4">
            ...
            {links.map((link) => {
              ...
              return (
                <button
                  ...
                  className={`relative flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-medium transition-all duration-300 group ${
                    isActive
                      ? "text-white"
                ...
                >
                  {isActive && (
                    <span className="absolute inset-0 rounded-xl bg-gradient-to-r from-violet-600/30 to-indigo-600/30 border border-violet-500/30" />
                  )}
                  <span className={`text-xs ${isActive ? "text-violet-400" : "text-muted-foreground group-hover:text-violet-400"} transition-colors`}>
                    {link.icon}
                  </span>
        ...
        {/* Mobile Menu */}
        <div className={`md:hidden transition-all duration-300 overflow-hidden ${menuOpen ? "max-h-64 pb-4" : "max-h-0"}`}>
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm font-medium text-left transition-all ${
                    isActive ? "bg-violet-600/20 text-white border border-violet-500/30" : "text-muted-foreground hover:text-white hover:bg-white/5"
                  }`}
                >
